...
class glue(object):
	...
	def readspecbin(self,filename,NWL=int(12000),NLINES=int(10000000)):
		...
		ll['LABEL']    = np.array([' {0:11s} '.format(x)[:-1] for x in LABELin],dtype=str)
		...
		ll['LABELP']   = np.array([' {0:11s} '.format(x)[:-1] for x in LABELPin],dtype=str)
		...
		ll['LABELP']   = np.array([' {0:5s} '.format(x) for x in REFin],dtype=str)
		ll['NBLO']     = NBLOin
		ll['NBUP']     = NBUPin
		ll['ISO1']     = ISO1in
