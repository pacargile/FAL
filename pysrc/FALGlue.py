...
class glue(object):
	...
	def readspecbin(self,filename,NWL=int(12000),NLINES=int(1000000)):
		...
		print([''.join(LABELin[i,:].tostring().decode('ascii')) for i in range(NLINES)])
		ll['LABEL']    = np.array([''.join(LABELin[i,:].tostring('F')) for i in range(NLINES)])
		ll['EP']       = np.array(['{0:12.3f}'.format(x) for x in EPin],dtype='float')
		ll['XJP']      = np.array(['{0:5.1f}'.format(x) for x in XJPin],dtype='float')
		ll['LABELP']   = np.array([''.join(LABELPin[i,:].tostring('F')) for i in range(NLINES)])
		...
		ll['OTHER1']   = np.array([''.join(OTHER1in[i,:].tostring('F')) for i in range(NLINES)])
		ll['OTHER2']   = np.array([''.join(OTHER2in[i,:].tostring('F')) for i in range(NLINES)])
