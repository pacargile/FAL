...
C      REWIND 7
C      REWIND 9
      WRITE(6,*)"READING FORT.9 RUN INFO"
...
C     IF(PRDDOP.GT.0.)CALL PRDJNU(RHOXJ)
      IEDGE=1
      WRITE(6,*) 'READING ASYNTH FROM FORT.9'
      DO 25 NU=NULO,NUHI
C     NUPRD=NU
...
C      IF(NU.GT.ABS(LINOUT))GO TO 230
C      IF(MU.GT.1)GO TO 230
C      IRESID=RESID*1000.+.5
C      IPLOT=RESID*SLOPE+ORIGIN
C      IPLOT=MAX0(1,MIN0(101,IPLOT))
C      APLOT(IPLOT)=1HX
...
C
      ...
      NMU=1
      MU=1
      WRITE(6,*)"READ FORT.9 NLINES"
      READ(9)NLINES
      N910=NLINES+N10
      WRITE(7)N910
CCCCCCC
C  WRITE LINE INFO INTO AN ASCII FILE
      OPEN(UNIT=999,FILE="LINEINFO.dat",STATUS='NEW',FORM='FORMATTED')
      WRITE(999,*)'NLINES'
 9999 FORMAT(I15)
      WRITE(999,9999)N910
      CLOSE(UNIT=999)
CCCCCCC
   ...
   80 FORMAT(3I10)
      ...
      WAVOLD=0.
      IEDGE=1
      WRITE(6,*)'READ FORT.9 LINDAT8,LINDAT,ALINEC'
C      DO 90 ILINE=1,NLINES
      DO WHILE (1.EQ.1)

      READ(9,END=90)LINDAT8,LINDAT,ALINEC
      WAVE=WLVAC
